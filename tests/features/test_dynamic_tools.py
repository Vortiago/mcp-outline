...
def _build_maps():
    ...
    register_all(_mcp)
    return build_tool_endpoint_map(_mcp)


_ENDPOINT_MAP = _build_maps()
...
@pytest.mark.anyio
async def test_no_blocked_tools_shows_all(fresh_mcp_server):
...
@pytest.mark.anyio
async def test_scoped_key_hides_blocked_tools(fresh_mcp_server):
    """Scoped key blocking write endpoints hides write tools.

    Uses the lowlevel handler path to match real MCP clients.
    """
    # Build a blocked set mimicking what a read-only scope
    # would produce
    write_tools = {
        "create_document",
        "update_document",
...
async def test_get_blocked_tools_no_api_key():
    ...
    result = await get_blocked_tools(None, None, _ENDPOINT_MAP)
    assert result == set()
...
async def test_get_blocked_tools_empty_string_api_key():
    ...
    result = await get_blocked_tools("", None, _ENDPOINT_MAP)
    assert result == set()
