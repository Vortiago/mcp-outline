...
| **Claude Code Plugin** | `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.mcp.json`, `hooks/` | Repo discovery | Install: `/plugin marketplace add Vortiago/mcp-outline` |
...
- **`hooks/check_outline_config.py`** — Checks `OUTLINE_API_KEY` is set, prints setup instructions if not.
